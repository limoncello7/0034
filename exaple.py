...
html.P("This chart shows the changes in Brent & WTI oil prices."),
dcc.Graph(id='line-daily', figure=fig_1),
html.P("This chart shows the trend of Brent price changes compared to gas price changes."),
html.Div(
[
...
@app.callback(
    Output('pattern-matching-graph', 'figure'),
    [Input('category-dropdown', 'value'),
     Input('time-radio', 'value')]
)
def update_graph(category, time):
    if category == 'gas':
        df = df_gas_daily
        title = 'Gas Prices'
    elif category == 'wti':
    ...
    return {
        ...
        'layout': go.Layout(
            title=title,
            xaxis={'title': 'Date' if time == 'daily' else time.title()},
